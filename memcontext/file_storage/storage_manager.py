...
class FileStorageManager:
    ...
    def find_file_by_name(self, filename: str, file_type: Optional[FileType] = None) -> Optional[FileRecord]:
        """
        通过文件名查找文件记录
        
        Args:
            filename: 文件名（original_filename）
            file_type: 文件类型过滤（可选）
        
        Returns:
            文件记录对象，不存在返回None。如果有多个同名文件，返回第一个匹配的。
        """
        for file_id, data in self.metadata_index.items():
            record = FileRecord.from_dict(data)
    ...
    def find_files_by_name(self, filename: str, file_type: Optional[FileType] = None) -> List[FileRecord]:
        """
        通过文件名查找所有匹配的文件记录（可能有多个同名文件）
